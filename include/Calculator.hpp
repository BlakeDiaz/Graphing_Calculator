...
class Calculator
{
  public:
    ...
    static std::tuple<std::unordered_set<char>, Parse_Error> locate_user_function_dependencies(const std::string& expression, int row_number = 1);
    static std::tuple<std::string, Parse_Error> format_expression(std::unordered_map<char, User_Function>& user_function_map,
                                         std::string expression, int row_number = 1);
    static std::tuple<double, Parse_Error> solve_expression(std::string formatted_expression, int row_number = 1);
};
